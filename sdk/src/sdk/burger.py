...
class Burger:
    ...
    def create_burger(self, request: shared.BurgerCreate, retries: Optional[utils.RetryConfig] = None) -> operations.CreateBurgerResponse:
        ...
        if http_res.status_code == 201:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.BurgerOutput])
    ...
    def list_burgers(self, retries: Optional[utils.RetryConfig] = None) -> operations.ListBurgersResponse:
        ...
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[list[shared.BurgerOutput]])
                res.burger_outputs = out
            else:
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
        elif http_res.status_code >= 400 and http_res.status_code < 500 or http_res.status_code >= 500 and http_res.status_code < 600:
            raise errors.SDKError('API error occurred', http_res.status_code, http_res.text, http_res)
    ...
    def read_burger(self, request: operations.ReadBurgerRequest, retries: Optional[utils.RetryConfig] = None) -> operations.ReadBurgerResponse:
        r"""Read Burger
        Read a burger
        """
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
